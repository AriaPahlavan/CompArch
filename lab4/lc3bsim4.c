...
void updateVector(System_Latches curLatch) {
	...
	switch(VECTORMUX) {
		case HEX_ONE_:   result = LSHF(SEXT(curLatch.INTV, 8)); break;
...
void updatePriv(System_Latches curLatch) {
	logging(S, "Updating Priv to: ",
	        Ix, Low16bits(bit(BUS,15)), debug);

	NEXT_LATCHES.Priv = Low16bits(bit(BUS,15));
}
...
void setCC() {
	...
	switch(PsrMux) {
		case LOGIC_:
			...
			break;
		case BUS_:
			logging(S, "Setting CC to N=", Id, bit(BUS, 2),
			        S, " Z=", Id, bit(BUS, 1),
